...
const dummyData = {
  ...
  entities_summary: [
    ...
    // duplicated blocks retained intentionally per original data
    {
      name: "Customer",
      metrics: {
        count: { pass: 1200, fail: 300, error: 50 },
...
const SummaryTab = () => {
  ...
  const CustomTooltip = ({
    ...
    return (
      ...
      >
        ...
        {rules.length > 0 ? (
          <div className="rules-chart-wrap">
            <div className="rules-chart-main">
              <ResponsiveContainer
                width="100%"
            ...
            <div className="rules-chart-xaxis">
  ...
  function filterEntitiesSummary(term) {
    if (!term.trim()) return dummyData.entities_summary;
    const q = term.toLowerCase();

    return dummyData.entities_summary
      .map((entity) => {
        const entityMatch = entity.name.toLowerCase().includes(q);
        if (entityMatch) return entity;
        ...
        if (matchedRules.length > 0) {
          return {
            ...entity,
            metrics: { ...entity.metrics, rules: matchedRules },
          };
        }
        return null;
      })
          ...
          <div className="donut-chart-block">
            <ResponsiveContainer width="60%" height={250}>
              <PieChart>
                <Pie
                  ...
                  outerRadius={100}
                  label={false}
                  onClick={(d) => setSelectedStatus(d.key)}
                >
            ...
            <div className="donut-labels">
              ...
              {getOverallScoreData().map((entry) => (
                ...
                >
                  <span style={{ backgroundColor: COLORS[entry.key] }} />
                  {entry.name}
                </div>
              ))}
            </div>
          ...
          <div className="donut-chart-block">
            <ResponsiveContainer width="60%" height={250}>
              <PieChart>
                <Pie
                  ...
                  outerRadius={100}
                  label={false}
                  onClick={(d) => setSelectedDimension(d.name)}
                >
            ...
            <div className="donut-labels">
              {getMetricsData().map((entry) => (
                ...
                >
                  <span style={{ backgroundColor: COLORS[entry.name] }} />
